...
export default function PortalStudentsPage() {
  ...
  const handleToggleBlock = async (student: Student) => {
    try {
      ...
      await portalAPI.toggleBlock(student.id, !student.is_blocked);
      fetchStudents();
    } catch (e: unknown) {
  ...
  const activeCount = students.filter((s) => !s.is_blocked).length;
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Students registered via your secret codes
          </p>
        </div>
        <button
          ...
          <div className="divide-y">
            {filtered.map((student) => (
              ...
              >
                ...
                {student.secret_code && (
                  <span className="hidden sm:block text-xs font-mono px-2 py-1 rounded bg-muted">
                    {student.secret_code}
                  </span>
                )}
